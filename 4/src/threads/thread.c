...
void thread_exit(void)
{
  ...
  /* Free all memory allocAed for owned files. */
...
init_thread(struct thread *t, const char *name, int priority)
{
  ...
  /* Init list for OWNED_FILES. */
  list_init(&t->owned_files);
  /* The next file descriptor begins at 2. */
  t->next_fd = 2;
...
/* Add FILE into the current thread's file list and return the file descriptor. */
int thread_add_file(struct file *file, struct dir *dir)
{
  struct thread *t = thread_current();
...
/* Close the file according to the file descriptor FD. */
