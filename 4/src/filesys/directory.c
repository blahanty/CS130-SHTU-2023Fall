...
/* /NEW */
{
  ...
  if (is_dir)
  {
    struct dir *dir_added = dir_open(inode_open(inode_sector));
    if (!dir_added)
      goto done;

    e.inode_sector = inode_get_inumber(dir_get_inode(dir));
    e.in_use = false;
    if (inode_write_at(dir_added->inode, &e, sizeof(struct dir_entry), 0) != sizeof(struct dir_entry))
    {
      dir_close(dir_added);
      goto done;
    }
...
bool dir_remove(struct dir *dir, const char *name)
{
  ...
  if (inode->data.is_dir)
  {
    ...
    for (offset = 0; inode_read_at(dir_opened->inode, &temp_e, sizeof temp_e, offset) == sizeof(struct dir_entry); offset += sizeof(struct dir_entry))
      if (temp_e.in_use)
      {
...
void dir_split_path(const char *path, char *dir, char *file)
{
  size_t length = strlen(path) + 1;
  char path_[length];
  char *dir_ = dir;
  strlcpy(path_, path, length);
  ...
  for (token = strtok_r(path_, "/", &save_ptr); token; token = strtok_r(NULL, "/", &save_ptr))
  {
    length = strlen(last_token);
    if (dir_ && length)
    {
      memcpy(dir_, last_token, length);
      dir_ += length;
      *dir_ = '/';
      ++dir_;
    }
  ...
  *dir_ = '\0';
  memcpy(file, last_token, strlen(last_token) + 1);
}
...
struct dir *dir_open_path(const char *path)
{
  ...
  char path_[length];
  strlcpy(path_, path, length);
  struct dir *cur_dir, *next_dir;
  if (path[0] == '/')
    /* Root directory. */
    cur_dir = dir_open_root();
  else
    cur_dir = thread_current()->cur_dir ? dir_reopen(thread_current()->cur_dir) : dir_open_root();

  char *save_ptr;
  for (char *token = strtok_r(path_, "/", &save_ptr); token; token = strtok_r(NULL, "/", &save_ptr))
  {
    struct inode *inode;
