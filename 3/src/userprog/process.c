...
void process_exit(void)
{
  ...
  while (!list_empty(&cur->mmap_list))
    munmap(list_entry(list_begin(&cur->mmap_list), struct mmap_entry, elem)->id);
...
bool load(const char *file_name, void (**eip)(void), void **esp)
{
  ...
  /* NEW */
  bool hash_success = hash_init(&t->page_table, page_table_entry_hash, hash_less, NULL);
  if (!hash_success)
    goto done;
  /* /NEW */
             ...
             uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
  ...
  while (read_bytes > 0 || zero_bytes > 0)
  {
    ...
    /* NEW */
    uint8_t *kpage = frame_get_page(PAL_USER | PAL_ZERO, upage);
    /* /NEW */
    if (kpage == NULL)
      return false;
...
setup_stack(void **esp)
{
  ...
  bool success = false;
  /* NEW */
  kpage = frame_get_page(PAL_USER | PAL_ZERO, PHYS_BASE - PGSIZE);
  /* /NEW */
  if (kpage != NULL)
  {
