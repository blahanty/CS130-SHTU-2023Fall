...
bool thread_mlfqs;
/* NEW */
fp load_avg; /* System load average. */
/* /NEW */
...
/* P1T2 main function to create thread */
...
/* P1T2 main function to control thread */
...
void thread_set_nice(int nice UNUSED)
{
  ...
  current_thread->priority = FP_TO_INT(FP_SUB_INT(FP_SUB(INT_TO_FP(PRI_MAX), FP_DIV_INT(current_thread->recent_cpu, 4)), 2 * current_thread->nice));
  current_thread->priority = current_thread->priority < PRI_MIN ? PRI_MIN : current_thread->priority;
  current_thread->priority = current_thread->priority > PRI_MAX ? PRI_MAX : current_thread->priority;
...
/* NEW */
/* Check and unblock thread T if it should wake up. */
void thread_wake_up(struct thread *t, void *aux UNUSED)
{
  ASSERT(is_thread(t));

  if (t->status == THREAD_BLOCKED && t->ticks_to_sleep > 0 && !--t->ticks_to_sleep)
    thread_unblock(t);
}

/* Compare the priority of threads in waiting list. */
...
/* Donate DONATOR priority to HOLDER. */
void thread_donate_priority(struct thread *donator, struct thread *holder)
{
  ASSERT(is_thread(donator));
  ASSERT(is_thread(holder));

  donator->lock_wait->priority = donator->priority;
  holder->priority = donator->priority;

  if (holder->lock_wait)
  {
    struct thread *holder_holder = holder->lock_wait->holder;
    if (holder_holder && holder_holder->priority < holder->priority)
      thread_donate_priority(holder, holder_holder);
  }
}

/* Recover priority of thread T. */
void thread_recover_priority(struct thread *t)
{
  ASSERT(is_thread(t));
  ...
  {
    int max_priority = list_entry(list_front(&t->lock_hold), struct lock, elem)->priority;
    t->priority = max_priority > t->original_priority ? max_priority : t->original_priority;
  }
}

/* Check if thread T is idle. */
...
/* Update load_avg, recent_cpu, and priority of all threads. */
void thread_update_if_mlfqs(void)
{
  ASSERT(thread_mlfqs);
  ...
  for (struct list_elem *e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e))
  {
    ...
    if (t != idle_thread)
    {
      ...
      t->priority = FP_TO_INT(FP_SUB_INT(FP_SUB(INT_TO_FP(PRI_MAX), FP_DIV_INT(t->recent_cpu, 4)), 2 * t->nice));
